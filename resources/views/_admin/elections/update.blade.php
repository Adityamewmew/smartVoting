...
@section('content')
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="bg-white overflow-hidden shadow-lg rounded-2xl dark:bg-neutral-800 border-2 border-gray-100 dark:border-neutral-700">
            ...
            <form id="update-form" class="p-6" navigate-form action="{{ route('admin.elections.doUpdate', $data->id) }}" method="POST">
                @csrf
                <div class="space-y-4">
                    {{-- Name --}}
                    <div>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                            ...
                            placeholder="Opsional: Penjelasan singkat tentang pemilihan ini">{{ old('description', $data->description) }}</textarea>
                        @error('description')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="start_time" name="start_time" value="{{ old('start_time', \Carbon\Carbon::parse($data->start_time)->format('Y-m-d H:i')) }}"
                        ...
                        <div>
                            ...
                            <input type="text" id="end_time" name="end_time" value="{{ old('end_time', \Carbon\Carbon::parse($data->end_time)->format('Y-m-d H:i')) }}"
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                            ...
                            @foreach ($statuses as $val => $label)
                                <option value="{{ $val }}" {{ old('status', $data->status) == $val ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
